...
| `fix-casos-copy.ts`                             | Fix footnote artifacts in servicios subtitles ("…reales.2" → "…reales.") + report casos metrics/titles that duplicate words when concatenated (renderer already sanitizes via `lib/utils/case-copy.ts`; curate in admin)                                                            |
