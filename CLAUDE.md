...
- **El hub de IA y las 4 páginas de ingeniería de agentes NO leen su fila de `servicios`**: el copy está en [lib/content/agentes.ts](./lib/content/agentes.ts) y `scripts/seed-servicios-agentes.ts` lo copia a la BD. Editar esas filas en el admin no cambia las páginas. En esa línea no se publican cifras de resultados. Ver [docs/03-cms-data-model.md](./docs/03-cms-data-model.md).
- **Enlaces internos: `LocaleLink`, nunca `next/link` plano** en páginas de marketing y componentes compartidos. Las rutas se escriben en ES y `LocaleLink` las traduce en /en. Con `next/link` el visitante de /en termina en el sitio en español. Ver [docs/02-routes.md](./docs/02-routes.md).
