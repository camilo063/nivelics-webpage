  ...
  return (
    <section className="border-y border-white/5 bg-[#0A0A0F] py-12 md:py-16">
      <div className="mx-auto max-w-6xl px-6">
        <p className="text-center text-xs font-semibold uppercase tracking-widest text-text-40">
          {title}
        </p>
        <div className="mt-8 flex flex-wrap items-center justify-center gap-x-10 gap-y-6 md:gap-x-14">
          {logos.map((logo, i) => (
            <div key={i} className="text-lg font-bold tracking-tight text-text-55 md:text-xl">
              {logo}
            </div>
          ))}
        </div>
